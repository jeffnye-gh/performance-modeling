...
//  }
  ...
  */ 
  ...
  //
  // CSR is sensitive to periodic patterns in global history 
  ...
  for(int j = 0; j < 2 ; j++) {
    for(int i=0; i < NUMTAGTABLES; i++) {
      ...
      if(j == 0) {
        ...
        if(i < 2) tagComp[j][i].targetLength = 9 ;
        else      tagComp[j][i].targetLength = 9 ;    

      } else {

        if(i < 2) tagComp[j][i].targetLength = 8 ;
        else      tagComp[j][i].targetLength = 8 ;
      }
    }   
  }    
...
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
bool Tage::GetPrediction(UINT32 PC) {
  ...
  // Hash to get tag includes info about bank, pc and global history 
  // compressed
  ...
  /* Old comment
    indexTagPred[0] = PC ^ (PC >> TAGPREDLOG) ^ indexComp[0].compHist
                         ^ PHR ^ (PHR >> TAGPREDLOG);
    indexTagPred[1] = PC ^ (PC >> TAGPREDLOG) ^ indexComp[1].compHist
                         ^ PHR ^ (PHR >> TAGPREDLOG);
    indexTagPred[2] = PC ^ (PC >> TAGPREDLOG) ^ indexComp[2].compHist
                         ^ PHR ^ (PHR >> TAGPREDLOG);
    indexTagPred[3] = PC ^ (PC >> TAGPREDLOG) ^ indexComp[3].compHist
                         ^ PHR ^ (PHR >> TAGPREDLOG);
  */

  // New statements - slight reformat from the original
  //  indexTagPred[0] = PC ^ (PC >> (TAGPREDLOG - 0)) ^ indexComp[0].compHist 
  //                       ^ PHR ^ (PHR >> TAGPREDLOG);
  //  indexTagPred[1] = PC ^ (PC >> (TAGPREDLOG - 1)) ^ indexComp[1].compHist 
  //                       ^ (PHR );
  //  indexTagPred[2] = PC ^ (PC >> (TAGPREDLOG - 2)) ^ indexComp[2].compHist 
  //                       ^ (PHR & 31);
  //  indexTagPred[3] = PC ^ (PC >> (TAGPREDLOG - 3)) ^ indexComp[3].compHist
  //                       ^ (PHR & 7);

  // refactored new statements
  indexTagPred[0] = HASH(PC,0,indexComp[0].compHist, PHR ^ (PHR >> TAGPREDLOG));
  indexTagPred[1] = HASH(PC,1,indexComp[1].compHist, PHR&0xFFFF);
  indexTagPred[2] = HASH(PC,2,indexComp[2].compHist, PHR&0x1F  );
  indexTagPred[3] = HASH(PC,3,indexComp[3].compHist, PHR&0x07  );
                            ...
                            bool predDir, UINT32 branchTarget)
{
  ...
  if (primeBank < NUMTAGTABLES) {
    ...
    if ((predDir != altPred)) {
      ...
      if (predDir == resolveDir) {
        ...
        tagPred[primeBank][indexTagPred[primeBank]].usefulBits
          = SatIncrement(tagPred[primeBank][indexTagPred[primeBank]].usefulBits,
                         BIMODAL_CTR_MAX);

      } else {

        tagPred[primeBank][indexTagPred[primeBank]].usefulBits
          = SatDecrement(tagPred[primeBank][indexTagPred[primeBank]].usefulBits);
      }
    ...
    if(resolveDir) {
      ...
      tagPred[primeBank][indexTagPred[primeBank]].ctr
        = SatIncrement(tagPred[primeBank][indexTagPred[primeBank]].ctr,
                       TAGPRED_CTR_MAX);

    } else {

      tagPred[primeBank][indexTagPred[primeBank]].ctr
        = SatDecrement(tagPred[primeBank][indexTagPred[primeBank]].ctr);
    }
    ...
    if(resolveDir) {
      ...
      bimodal[bimodalIndex]
        = SatIncrement(bimodal[bimodalIndex], BIMODAL_CTR_MAX);
  ...
  // Check if the current Entry which gave the prediction is a newly 
  // allocated entry or not.
  if (primeBank < NUMTAGTABLES) {
  ...
  // Periodic Useful bit Reset Logic ( Important so as to optimize 
  // compared to PPM paper)
...
void    Tage::TrackOtherInst(UINT32 PC, OpType opType, UINT32 branchTarget){
